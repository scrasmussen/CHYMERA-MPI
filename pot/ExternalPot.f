...
#endif
! Use the position of the star as calculated by wiggle.F.
...
c     &+indirecty*rhf(J)*sin((dble(L-1)+half)*dtheta)
                     ...
                     phi(j,k,l)=phi(j,k,l)
     &+(indirectx2*rhf(J)+indirecty2*(dble(L-1)+half)*dtheta)
     &+mass_star*(indirectx*rhf(J)
     &*cos((dble(L-1)+half)*dtheta)/sqrt(rhf(J)**2+zhf(k)**2)**3
     &+(indirecty2*rhf(J)-indirectx2*(dble(L-1)+half)*dtheta)
     &+indirecty*rhf(J)
     &*sin((dble(L-1)+half)*dtheta)/sqrt(rhf(j)**2+zhf(k)**2)**3
     &+half*(indirectx**2+indirecty**2)/sqrt(rhf(J)**2+zhf(k)**2)**3)
