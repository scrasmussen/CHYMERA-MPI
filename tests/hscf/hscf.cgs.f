C****************************************************************************/
c   Conversion to cgs units starts on 350,570
c
c
...
c     prmtr.h contains the grid dimensions
     ...
     4           xmas(jmax1),omega(jmax1),r(jmax1)
      ...
      dimension anngy(jmax1,jmax1),cen_pot(jmax1)
     ...
     &             jmax2,jmax1,kmax2,np)
         ...
         do j = 2,jmax1
           do k = 2,kmax1
             ...
             phi_star(j,k) = - g * starm / radius
             phi(j,k)      =  g * phi(j,k) + phi_star(j,k)
           enddo
         enddo
...
c ======================================================================
c
c   transform code variables to cgs and polytrope units
c
c=======================================================================
c
...
c
      if ((case.eq.1).and.(pin.ne.0.0)) then
        ...
        re = (8.0*a*sqrt(1.0 + xmax**2.0)/(b*g*rhomax*
     1       hmax(nfin)))**0.5
      endif
...
c
      ...
      tow = tt*aww
c
c     vc is the virial, a measure of the numerical
...
c
     ...
     &              tt,-ww,-ws,3.0*ss,starm/tmss
        ...
        write(13,111) rb,tmss,starm,tt,-ww,-ws,3.*ss,re
...
c     zero out rhos not considered in hydro code

      rhocut = 0.0
      do j = 2,jmax2
        if(rho(j,2) .gt. rhocut) rhocut = rho(j,2)
      enddo
      rhocut = 1.0e-10*rhocut 
      do j = 2,jmax2
         do k = 2,kmax2
           if(rho(j,k) .lt. rhocut) rho(j,k) = 0.d0
         enddo
      enddo
...
c     factor for omega
      ...
      do j=2,jmax1
         if(omegac(j) .gt. omegac(j-1)) then
            omax = omegac(j)
            omaxj = j
         endif   
      enddo
...
c     should really set to zero for protostars
      ...
      oslope = (omegac(3) - omegac(2))/(rhf(3) - rhf(2))
      ob = omegac(2) - oslope*rhf(2)
...
c
     ...
     &   ((anngy(j+1,2)-anngy(j,2))/dr)
        ...
        if(surface.ne.0.0) then
          ...
          vortensity=(2.0+np)*omegac(j)/surface
          write(6,1061) j,rhf(j),rho(j,2),omegac(j),xmas(j),phi(j,2)
     &                  -phi_star(j,2),anngy(j,2)
...
c
 ...
 96   format(1p6e13.5)
      ...
      write(93,195) pin,jtot,r(jout),omegac(jrhomax),rho(jrhomax,2)
     &            ,tow,dr,dr,AINEWZ,jout,kout
...
c     incorporates sm directly for automatic plotting.
