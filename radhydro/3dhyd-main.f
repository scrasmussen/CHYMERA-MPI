...
! This version of the code is called CHYMERA
!
! Particles have been added using the module particles/particle_module.f
!
! Binary potential is possible using the Roche potential (Default), as well
! as the Boss (2004) approach.  
!
! Nora Bolig (ACB)
!
...
!                    table. (source.f) ACB
!     *engfind: find eng based on temp for a single cell
!     *eos is now determined completely by new eos.f90 module. 
...
10000    format('step=',i8,'   time=',1pe26.13,'   delt=',1pe26.15)  !dkb
     ...
     &       vx_p(1),vy_p(1),vz_p(1)
         ...
         IHEAD=-1
         IPRINT=1
         if(MOD(ITSTEP,ISTOR).EQ.0.and.ITSTEP.NE.ITSTRT)
     &      cALL RITE(2,-1,ITSTOP,ITSTRT,ITSTOP,ISTOR,1,1,1,1,1)
         IF(MOD(ITSTEP,IDIAG).NE.0.and.ITSTEP.NE.ITSTRT) then
            IPRINT=0
            IHEAD=0
         end if
...
!$OMP DO SCHEDULE(STATIC)
         DO L=1,LMAX
           DO K=1,KMAX2
             DO J=1,JMAX2
                PPASSFLUX(J,K,L,I)=PASSFLUX(J,K,L,I)
             enddo
           enddo
         enddo
!$OMP ENDDO
...
#if PASSIVE>0
         CALL FLUX(S,T,A,RHO,EPS,PASSFLUX)
#else
...
#if PASSIVE>0
         CALL FLUX(SS,TT,AA,RRHO,EEPS,PPASSFLUX)
#else
...
!$OMP DO SCHEDULE(STATIC)
         DO L=1,LMAX
           DO K=1,KMAX2
             DO J=1,JMAX2
                PASSFLUX(J,K,L,I)=PPASSFLUX(J,K,L,I)
             enddo
           enddo
         enddo
!$OMP ENDDO
...
!$OMP END PARALLEL
     ...
     &     phi_star,rpstar*cos(phi_star),rpstar*sin(phi_star)
      ...
      tmass=zero;indirectx=zero;indirecty=zero
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         IF(MOD(itstep,IDIAG).EQ.0.and.ITSTEP.NE.ITSTRT) THEN
            ...
            rhofile='passive.'//tim
            OPEN(UNIT=14,FILE=rhofile,FORM='UNFORMATTED')
            write(14)JMAX,KMAX,LMAX,PAS
            write(14)passflux(1:JMAX2,1:KMAX2,1:LMAX,1:PAS)
            write(14)time
            close(14)
#endif
